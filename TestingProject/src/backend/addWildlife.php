...
if (
    ...
    $stmt = $conn->prepare("INSERT INTO wildlife (species_name, common_name, scientific_name, population_estimate, status) 
                             VALUES (?, ?, ?, ?, ?)");
    if ($stmt) {
        $stmt->bind_param("sssis", $species_name, $common_name, $scientific_name, $population_estimate, $status);
        if ($stmt->execute()) {
